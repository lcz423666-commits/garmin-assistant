...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        gm.record_rate_limit_backoff("RegressionUser", now_ts=1000.0)
        until = gm.get_rate_limit_backoff_until("RegressionUser", now_ts=1001.0)
        assert until is not None and int(until) == 8200, until

        user = {"name": "RegressionUser"}
        ...
        gm.clear_rate_limit_backoff("RegressionUser")
        assert gm.get_rate_limit_backoff_until("RegressionUser", now_ts=1001.0) is not None
        assert add_user.check_rate_limit_before_onboarding(user, now_ts=1001.0) is not None
        assert gm.get_rate_limit_backoff_until("RegressionUser", now_ts=8201.0) is None
